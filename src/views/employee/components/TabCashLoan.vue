    ...
    <template v-else>
      ...
      <div class="history-card">
        ...
        <div class="history-list">
          <div v-for="loan in loans" :key="loan.id" class="history-item">
          ...
          <p v-if="!loans.length" class="empty-text">No recent loans logged.</p>
        </div>
      </div>
    </template>
  </div>
...
function onRequest() {
  ...
  emit('openPinModal', { mode: 'loan', amount: amount.value, resetAmount: () => { amount.value = '' } })
}
</script>
...
.status-approved, .status-active { color: #10b981; }
