  ...
  async function submitProductionLog(data) {
    try {
      ...
      const payload = {
        ...
        divider_type: (data.dividerType && data.dividerType !== 'Other' && !isNaN(parseInt(data.dividerType))) ? parseInt(data.dividerType) : 0,
        placement_style: data.placement || '',
        size_cm: data.size ? parseInt(data.size) : 0,
